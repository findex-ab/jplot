...
export const linePlot = (lineConfig) => (args) => {
    ...
    const computeData = (args) => {
        ...
        const startAtZero = !!lineConfig.yAxis?.startAtZero &&
        ...
        let [xValues, yValues] = lineConfig.mergeDuplicates
            ? combine(keys, dataset.values, formatX, lineConfig.mergeMethod || EMergeMethod.SUM)
    ...
    const draw = (args, plotctx) => {
        ...
        yAxisPoints.forEach((p, i) => {
            ...
            ctx.beginPath();
            ctx.fillText(yTickLabels[clamp(i, 0, yTickLabels.length - 1)], p.x, p.y - 2);
            ctx.closePath();
        });
        // draw x-axis labels
        xAxisPoints.forEach((p, i) => {
    ...
    const updateTooltip = (args, plotctx) => {
        ...
        const nextTooltipPos = prevTooltipPos.lerp(point
            ...
            .add(VEC2(0, -(32 + 2 * cursorRadius + 0.5 * args.state.tooltip.rect.height))), clamp(args.app.state.time.delta * 8, 0, 1));
